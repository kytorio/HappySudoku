...
import { strategyMap } from '@sudoku/stores/strategy'
...
export function solveSudokuTest(sudoku) {
    ...
    let [initialPossibleGrid, initialReferenceGrid, initialStrategyName] = initialStrategyFunction(sudoku);
    let maxCellsBelowLevel = initialPossibleGrid.flat().filter(cell => cell.length <= level && cell.length > 0).length;
    let possibleNumbersGrid;
    ...
    function dfs(currentGrid, currentStrategyIndex, currentCombination) {
        ...
        for (let i = currentStrategyIndex; i < strategyKeys.length; i++) {
            ...
            let cellsBelowLevel = newPossibleGrid.flat().filter(cell => cell.length <= level && cell.length > 0).length;
    ...
    dfs(initialPossibleGrid, 1, [initialStrategyName]);
    ...
    for (let i = 0; i < bestStrategyCombination.length; i++) {
        ...
        let strategyFunction = strategyMap.get(key);
