...
class HiddenPairsStrategy extends Strategy{
    ...
    findHiddenPairs(cells) {
        ...
        pairMap.forEach((indices, key) => {
            if (indices.length === 2) {
                ...
                if (candidates.has(indexKey)) {
                    candidates.get(indexKey).push(parseInt(key));
                } else {
                    candidates.set(indexKey, [parseInt(key)]);
                }
            }
        });

        candidates.forEach((nums, indices) => {
            if (nums.length === 2) {
    ...
    processHiddenPairs(grid, reference) {
        for (let i = 0; i < SUDOKU_SIZE; i++) {
            // 行
            const row = grid[i];
            const rowPairs = this.findHiddenPairs(row);
            const rowExecuted = this.removeHiddenPairs(row, rowPairs);
            ...
            // 列
            const col = grid.map(row => row[i]);
            const colPairs = this.findHiddenPairs(col);
            const colExecuted = this.removeHiddenPairs(col, colPairs);
            colExecuted.forEach(([index1, index2]) => {
            ...
            const boxPairs = this.findHiddenPairs(box);
            const boxExecuted = this.removeHiddenPairs(box, boxPairs);
    ...
    execute(possibleNumberGrid) {
        ...
        [this.newPossibleNumberGrid, this.newReferenceGrid] = this.processHiddenPairs(this.newPossibleNumberGrid, this.newReferenceGrid);
        return [this.newPossibleNumberGrid, this.newReferenceGrid, this.strategy];
    }
}
            ...
            for (let row1 = 0; row1 < SUDOKU_SIZE; row1++) {
                for (let row2 = row1 + 1; row2 < SUDOKU_SIZE; row2++) {
                    ...
                    for (let col = 0; col < SUDOKU_SIZE; col++) {
                        if (this.newPossibleNumberGrid[row1][col].includes(num)) {
                            cols1.push(col);
                        }
                        if (this.newPossibleNumberGrid[row2][col].includes(num)) {
                            cols2.push(col);
                        }
                    }
                    ...
                    if (cols1.length === 2 && cols2.length === 2 && cols1.toString() === cols2.toString()) {
                        for (let r = 0; r < SUDOKU_SIZE; r++) {
                            if (r !== row1 && r !== row2) {
                                cols1.forEach(col => {
                                    if (this.newPossibleNumberGrid[r][col].includes(num)) {
                                        this.newPossibleNumberGrid[r][col] = this.newPossibleNumberGrid[r][col].filter(val => val !== num);
                                        this.newReferenceGrid[r][col].push([row1, col], [row2, col]);
                                    }
                                });
                            }
                        }
            ...
            for (let col1 = 0; col1 < SUDOKU_SIZE; col1++) {
                for (let col2 = col1 + 1; col2 < SUDOKU_SIZE; col2++) {
                    ...
                    for (let row = 0; row < SUDOKU_SIZE; row++) {
                        if (this.newPossibleNumberGrid[row][col1].includes(num)) {
                            rows1.push(row);
                        }
                        if (this.newPossibleNumberGrid[row][col2].includes(num)) {
                            rows2.push(row);
                        }
                    }
                    ...
                    if (rows1.length === 2 && rows2.length === 2 && rows1.toString() === rows2.toString()) {
                        for (let c = 0; c < SUDOKU_SIZE; c++) {
                            if (c !== col1 && c !== col2) {
                                rows1.forEach(row => {
                                    if (this.newPossibleNumberGrid[row][c].includes(num)) {
                                        this.newPossibleNumberGrid[row][c] = this.newPossibleNumberGrid[row][c].filter(val => val !== num);
                                        this.newReferenceGrid[row][c].push([row, col1], [row, col2]);
                                    }
                                });
                            }
                        }
